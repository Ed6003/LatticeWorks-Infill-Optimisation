...
%(*@\codesubsection{Create \dnt{Triply Periodic Minimal Surface}{TPMS}}{lattice-create-TPMS}@*)
...
distortedElements = [];
pattern = 'WARNING:\s*(\d+)\s*elements';  % regex pattern

% Read each line until distorted element warning is found
while ~feof(fid)
    tline = fgetl(fid);
